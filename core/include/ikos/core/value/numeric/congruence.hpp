...
//
//  /// \brief Create the congruence 0Z + n
//  explicit Congruence(FNumber n) : _is_bottom(false), _a(0), _b(std::move(n)) {}
//
//  /// \brief Create the congruence aZ + b
//  Congruence(FNumber a, FNumber b)
//      : _is_bottom(false), _a(std::move(a)), _b(std::move(b)) {
//    this->reduce();
//  }
//
...
//        // See http://www.dsi.unive.it/~avp/domains.pdf
//        // See https://math.stackexchange.com/a/1644698
//        FNumber lambda = (this->_b - other._b) / x;
//        FNumber sigma = this->_b - this->_a * u * lambda;
//        return Congruence(lcm(this->_a, other._a), sigma);
//      } else {
//        return bottom();
//      }
...
//
//  /// \brief Return true if the congruence contains n
//  bool contains(int n) const { return this->contains(FNumber(n)); }
//
//  /// \brief Return true if the congruence contains n
//  bool contains(const FNumber& n) const {
//    if (this->is_bottom()) {
//      return false;
//    } else if (this->_a == 0) {
//      return n == this->_b;
...
//  using CongruenceT = Congruence< FNumber >;
//
//  if (lhs.is_bottom() || rhs.is_bottom()) {
//    return CongruenceT::bottom();
//  } else {
...
//
///// \brief Modulo
//inline Congruence< FNumber > mod(const Congruence< FNumber >& lhs,
//                                 const Congruence< FNumber >& rhs) {
//  using CongruenceT = Congruence< FNumber >;
//
//  if (lhs.is_bottom() || rhs.is_bottom()) {
...
//
//    // aZ + b << a'Z + b' = (gcd(a, b * (2^a' - 1)))*(2^b')Z + b*(2^b')
//    FNumber x = 1 << rhs.residue();
//    FNumber y = 1 << rhs.modulus();
//    FNumber a = gcd(lhs.modulus(), lhs.residue() * (y - 1)) * x;
//    FNumber b = lhs.residue() * x;
//    return CongruenceT(a, b);
//  }
//}
//
...
/// \brief Congruence on unlimited precision floating points
