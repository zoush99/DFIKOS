...
/// \brief By zoush99
/// \brief Congruence on floating point
///
/// We do not try to define the congruence domain on rationals.
/// We instead implement this as the constant domain, for simplicity.
template <>
...
public:
  ...
  explicit Congruence(float n) : _cst(n) {}

  explicit Congruence(double n) : _cst(n) {}
...
//  }
  ...
  bool contains(float n) const { return this->_cst.contains(n); }

  bool contains(double n) const { return this->_cst.contains(n); }
